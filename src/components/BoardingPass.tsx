...
export function BoardingPass({ story }: { story: Story }) {
  return (
    <Link
      to="/stories/$slug"
        ...
        <div className="p-5">
          ...
          <div className="mt-3 flex items-baseline gap-3 font-mono text-2xl text-foreground">
            ...
            <svg viewBox="0 0 60 8" className="h-2 w-16 text-signal/60">
              <line x1="0" y1="4" x2="60" y2="4" stroke="currentColor" strokeWidth="0.5" strokeDasharray="3 3" />
              <text x="30" y="6" fontSize="7" textAnchor="middle" fill="currentColor">✈</text>
            </svg>
          ...
          <div className="mt-4 flex flex-wrap gap-2 font-mono text-[10px] uppercase tracking-wider text-muted-foreground/80">
            <span className="rounded-sm border border-border px-2 py-0.5">{story.industry.split("/")[0].trim()}</span>
            <span className="rounded-sm border border-border px-2 py-0.5">cover: {story.coverStory.split("/")[0].trim()}</span>
          </div>
        </div>
